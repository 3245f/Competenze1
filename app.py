...
def index():
    ...
    if request.method == "POST":
        ...
        normative_generiche = request.form.get("normative_generiche", "") 
        ...
        # livello di confidenza per i linguaggi di programmazione
        linguaggi_confidenza = []
        livelli_confidenza_tradotti = {
            "1": "base",
            "2": "principiante",
            "3": "intermedio",
            "4": "avanzato",
        ...
        all_languages = [
            "c", "java", "cpp", "python", "js", "html_css", "sql", "php",
            "ruby", "swift", "kotlin", "go", "rust", "typescript", "r",
            "matlab", "dart", "csharp","ada", "assembly", "cobol", "fortran", "vhdl", "verilog", "julia", "sas","erlang", "scala", "elixir",
            "react", "angular"
        ]

# Mappa i valori del checkbox ai nomi visualizzati per la colonna Excel
        ...
        for lang_code in all_languages:
            # Controlla se il checkbox del linguaggio è stato selezionato
            if request.form.get(f'language', None) and lang_code in request.form.getlist('language'):
                # Recupera il livello di confidenza per quel linguaggio
                confidence_level = request.form.get(f'{lang_code}-confidence')
                
                if confidence_level:
                    translated_level = livelli_confidenza_tradotti.get(confidence_level, "")
                    display_name = language_display_names.get(lang_code, lang_code.capitalize())
                    linguaggi_confidenza.append(f"{display_name}: {confidence_level} ({translated_level})")

        # Unisce tutti i linguaggi selezionati con i loro livelli di confidenza in una singola stringa
        autovalutazione_linguaggi_str = ", ".join(linguaggi_confidenza) 
...
# Progetti SVILUPPO
        ...
        for area in dettagli_sviluppo.keys():
            ...
            nome_progetto = request.form.getlist(f'nome_progetto_{area.lower()}[]') 
            ...
            nome_cliente = request.form.getlist(f'nome_cliente_{area.lower()}[]')  
            tipologia_progetto = request.form.getlist(f'tipologia_progetto_{area.lower()}[]') 
            durata = request.form.getlist(f'durata_{area.lower()}[]')
            descrizione = request.form.getlist(f'descrizione_{area.lower()}[]')
            normative = request.form.getlist(f'normative_{area.lower()}[]') 
            #print(f"{area} -> Linguaggi: {linguaggi}, Tool: {tool}, Ambito: {ambito}, Durata: {durata}, Descrizione: {descrizione}")
            esperienze = []
            for i in range(max(len(nome_progetto), len(linguaggi), len(tool), len(ambito), len(nome_cliente), len(tipologia_progetto), len(durata), len(descrizione), len(normative))):
                np = nome_progetto[i] if i < len(nome_progetto) else "" 
                l = linguaggi[i] if i < len(linguaggi) else ""
                t = tool[i] if i < len(tool) else ""
                a = ambito[i] if i < len(ambito) else "" 
                if a == 'Cliente':
                    nc = nome_cliente[i] if i < len(nome_cliente) else ""
                    tp = tipologia_progetto[i] if i < len(tipologia_progetto) else ""
                    cliente_info = f"Cliente: {nc} | Tipologia Progetto: {tp}"
                else:
                     cliente_info = f""

                e = durata[i] if i < len(durata) else ""
                d = descrizione[i] if i < len(descrizione) else ""
                n = normative[i] if i < len(normative) else "" 
                esperienze.append(f"PROGETTO: {np} | LINGUAGGI: {l} | TOOL: {t} | AMBITO: {a} | DETTAGLI {cliente_info} | DURATA: {e} | DESCRIZIONE: {d} | NORMATIVE: {n}")
            dettagli_sviluppo[area] =esperienze
            #print(dettagli_sviluppo)
...
# Progetti V&V
        ...
        for area in dettagli_vv.keys():
            ...
            nome_progetto = request.form.getlist(f'nome_progetto_{area}[]') 
            tecnologie = request.form.getlist(f'tecnologie_{area}[]')
            ambito = request.form.getlist(f'Ambito_{area}[]') 
            nome_cliente = request.form.getlist(f'nome_cliente_{area.lower()}[]')  
            tipologia_progetto = request.form.getlist(f'tipologia_progetto_{area.lower()}[]') 
            durata = request.form.getlist(f'durata_{area}[]')
            descrizione = request.form.getlist(f'descrizione_{area}[]')
            normative = request.form.getlist(f'normative_{area}[]') 
            #print(f"{area} -> Tecnologie: {tecnologie}, Ambito: {ambito}, Durata: {durata}, Descrizione: {descrizione}")
            esperienze = []
            for i in range(max(len(nome_progetto), len(tecnologie), len(ambito), len(nome_cliente), len(tipologia_progetto), len(durata), len(descrizione), len(normative))):
                np = nome_progetto[i] if i < len(nome_progetto) else ""  
                ...
                d = descrizione[i] if i < len(descrizione) else ""
                n = normative[i] if i < len(normative) else "" 
                esperienze.append(f"PROGETTO: {np} | TOOL: {t} | AMBITO: {a} | DETTAGLI {cliente_info} | DURATA: {e} | DESCRIZIONE: {d} | NORMATIVE: {n}")
...
# Progetti System
        ...
        for area in dettagli_system.keys():
            ...
            tecnologie = request.form.getlist(f'tecnologie_{area}[]')
            ambito = request.form.getlist(f'Ambito_{area}[]') 
            nome_cliente = request.form.getlist(f'nome_cliente_{area}[]')  
            tipologia_progetto = request.form.getlist(f'tipologia_progetto_{area}[]') 
            durata = request.form.getlist(f'durata_{area}[]')
            descrizione = request.form.getlist(f'descrizione_{area}[]')
            normative = request.form.getlist(f'normative_{area}[]') 
            #print(f"{area} -> Tecnologie: {tecnologie}, Ambito: {ambito}, Durata: {durata}, Descrizione: {descrizione}")
            esperienze = []
            for i in range(max(len(nome_progetto), len(tecnologie), len(ambito), len(nome_cliente), len(tipologia_progetto), len(descrizione), len(normative))):
                np = nome_progetto[i] if i < len(nome_progetto) else ""  
                t = tecnologie[i] if i < len(tecnologie) else ""
                a = ambito[i] if i < len(ambito) else "" 
                if a == 'Cliente':
...
# Progetti meccanica_elettronica 
        scelte_progetti_meccanica = []   
        dettagli_mec = {}
        nome_progetto_meccanica = request.form.getlist('nome_progetto_meccanica_elettronica[]')
        tecnologie = request.form.getlist('tool_meccanica_elettronica[]')
        ambito = request.form.getlist('Ambito_meccanica_elettronica[]')
        nome_cliente = request.form.getlist('nome_cliente_meccanica_elettronica[]')
        tipologia_progetto = request.form.getlist('tipologia_progetto_meccanica_elettronica[]')
        durata = request.form.getlist('durata_meccanica_elettronica[]')
        descrizione = request.form.getlist('descrizione_meccanica_elettronica[]')
        normative = request.form.getlist('normative_meccanica_elettronica[]') 
        esperienze = []
        progetti_mecc_elettr_hw_val = request.form.get('progetti_mecc_elettr_hw', 'No') 
        if progetti_mecc_elettr_hw_val == "Si":
            scelte_progetti_meccanica.append('meccanica_elettronica_elettromeccanica')
            for i in range(max(len(nome_progetto_meccanica), len(tecnologie),len(ambito), len(descrizione))):
                np = nome_progetto_meccanica[i] if i < len(nome_progetto_meccanica) else ""
                t = tecnologie[i] if i < len(tecnologie) else ""
                a = ambito[i] if i < len(ambito) else ""
                if a == 'Cliente':
                    nc = nome_cliente[i] if i < len(nome_cliente) else ""
                ...
                n = normative[i] if i < len(normative) else ""
                esperienze.append(f"PROGETTO: {np} | TOOL: {t} | AMBITO: {a} | DETTAGLI {cliente_info} | DURATA: {e} | DESCRIZIONE: {d} | NORMATIVE: {n}")
            dettagli_mec["meccanica_elettronica"] = esperienze
        #print("dettagli mec",dettagli_mec)
...
# Progetti BIM
        ...
        for area in dettagli_bim.keys():
            ...
            esperienze = []
            for i in range(max(len(nome_progetto), len(tool), len(ambito), len(nome_cliente), len(tipologia_progetto), len(descrizione), len(certificazione), len(normative))):
                np = nome_progetto[i] if i < len(nome_progetto) else "" 
                t = tool[i] if i < len(tool) else ""
                a = ambito[i] if i < len(ambito) else "" 
                if a == 'Cliente':
                ...
                esperienze.append(f"PROGETTO: {np} | TOOL: {t} | AMBITO: {a} | DETTAGLI {cliente_info} | DURATA: {e} | DESCRIZIONE: {d} | NORMATIVE: {n} | CERTIFICAZIONI: {c}")
...
# Progetti PM
        ...
        for area in dettagli_pm.keys():
            ...
            descrizione = request.form.getlist(f'descrizione_{area}[]')
            normative = request.form.getlist(f'normative_{area}[]') 
            #print(f"{area} -> Tool: {tool}, Ambito: {ambito}, nome_cliente {nome_cliente},  tipologia_progetto { tipologia_progetto}, Durata: {durata}, Descrizione: {descrizione}")
            esperienze = []
            for i in range(max(len(nome_progetto), len(tool), len(ambito), len(nome_cliente), len(tipologia_progetto), len(durata), len(descrizione), len(normative))): #mod
                np = nome_progetto[i] if i < len(nome_progetto) else ""  
                t = tool[i] if i < len(tool) else ""
                a = ambito[i] if i < len(ambito) else "" 
                if a == 'Cliente':
...
# Crea un dizionario 'data' (che viene poi convertito in DataFrame pandas) con tutte le informazioni raccolte dal form che devono essere salvate nel file excel
        ...
        aggiungi_sezione("Meccanica Elettronica", scelte_progetti_meccanica, dettagli_mec,data) 
